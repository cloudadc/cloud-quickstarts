...
// If Ingress Pod are in application pod
NS=test-10
...
NS=test-10 && IC_POD=$(kubectl get pods -n $NS -l app=nginx-ingress --no-headers | head -n 1 | awk '{print $1}') && kubectl exec $IC_POD -n $NS -- nginx -T 2>&1 | grep -v '^[[:space:]]*$'
----
...
----
// 1. Get cookie
% curl -H "Host: test10.example.com" http://$LB/coffee -v
...
% for i in {1..5} ; do curl -s -H "Host: test10.example.com" --cookie "srv_id=8755b11e774743dcb825f2681132b16d; expires=Sun, 20-Aug-23 16:01:35 GMT; max-age=3600; path=/coffee" http://$LB/coffee | grep "addr" ; echo ; done
...
// 3. Analysis
    ...
    3) The NGINX INGRESS CONTROLLER IP are 10.8.0.15 and 10.8.1.20
        ...
        $ kubectl get pods -n test-10 -l app=nginx-ingress -o wide --no-headers
    ...
    4) The Application has 3 pods, and IPs are 10.8.0.11, 10.8.0.13 and 10.8.1.15
        ...
        $ kubectl get pods -n test-10 -l app=coffee -o wide --no-headers
