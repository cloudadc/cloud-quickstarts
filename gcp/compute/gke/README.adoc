...
The private Google Kubernetes Engine (GKE) cluster:

* A type of VPC-native cluster.
* The nodes only have internal IP addresses, nodes and Pods are isolated from the internet by default.
...
----
gcloud container clusters create private-cluster --zone=us-central1-a --cluster-version=1.25.7-gke.1000 --num-nodes=2 --enable-private-nodes --master-ipv4-cidr=172.16.0.0/28 --enable-ip-alias
...
[source, bash]
.*2. describe private cluster*
----
gcloud container clusters describe private-cluster --zone=us-central1-c
----

* link:private-cluster.json[private-cluster.json]

[source, bash]
.*3. Overview the IP allocation*
----
$ gcloud compute networks subnets describe my-subnet-0 --region=us-central1 --format=json
{
  "creationTimestamp": "2023-04-23T08:40:27.198-07:00",
  "description": "auto-created subnetwork for cluster \"private-cluster\"",
  "fingerprint": "_jFMIrMAYaI=",
  "gatewayAddress": "10.108.208.1",
  "id": "7131829086890553220",
  "ipCidrRange": "10.108.208.0/22",
  "kind": "compute#subnetwork",
  "name": "my-subnet-0",
  "network": "https://www.googleapis.com/compute/v1/projects/playground-s-11-fb1cea00/global/networks/default",
  "privateIpGoogleAccess": true,
  "privateIpv6GoogleAccess": "DISABLE_GOOGLE_ACCESS",
  "purpose": "PRIVATE",
  "region": "https://www.googleapis.com/compute/v1/projects/playground-s-11-fb1cea00/regions/us-central1",
  "secondaryIpRanges": [
    {
      "ipCidrRange": "10.112.0.0/14",
      "rangeName": "gke-private-cluster-pods-03c5b83b"
    },
    {
      "ipCidrRange": "10.108.224.0/20",
      "rangeName": "gke-private-cluster-services-03c5b83b"
    }
  ],
  "selfLink": "https://www.googleapis.com/compute/v1/projects/playground-s-11-fb1cea00/regions/us-central1/subnetworks/my-subnet-0",
  "stackType": "IPV4_ONLY"
}
----
...
* Node CIRD Range: `10.108.208.0/22`
* Pod CIRD Range: `10.112.0.0/14`
* Service CIRD Range: `10.108.224.0/20`
* Master CIRD Range: `172.16.0.32/28`
* privateEndpoint: `172.16.0.34`
* publicEndpoint: `34.66.239.59`
...
[source, bash]
----
gcloud container clusters create standard-cluster --num-nodes=2 --zone=us-central1-a --enable-ip-alias
gcloud container clusters get-credentials standard-cluster --zone=us-central1-a
----
...
[source, bash]
----
kubectl apply -f services/dns.yaml
kubectl apply -f services/backend-v1.yaml
kubectl apply -f services/backend-v2.yaml
----

* link:services/dns.yaml[services/dns.yaml]
* link:services/backend-v1.yaml[services/backend-v1.yaml]
* link:services/backend-v2.yaml[services/backend-v2.yaml]
...
.*1. Service lookup*
----
$ kubectl run test-ns --image=busybox:1.28 --rm -it --restart=Never -- nslookup dns-demo.nginx.svc.cluster.local
Server:    10.12.0.10
Address 1: 10.12.0.10 kube-dns.kube-system.svc.cluster.local

Name:      dns-demo.nginx.svc.cluster.local
Address 1: 10.8.0.9 dns-demo-2.dns-demo.nginx.svc.cluster.local
Address 2: 10.8.1.5 dns-demo-1.dns-demo.nginx.svc.cluster.local
pod "test-ns" deleted
----

[source, bash]
.*2. Service lookup(short name)*
----
$ kubectl run test-ns --image=busybox:1.28 --rm -it --restart=Never -- nslookup  dns-demo.nginx
Server:    10.12.0.10
Address 1: 10.12.0.10 kube-dns.kube-system.svc.cluster.local

Name:      dns-demo.nginx
Address 1: 10.8.1.5 dns-demo-1.dns-demo.nginx.svc.cluster.local
Address 2: 10.8.0.9 dns-demo-2.dns-demo.nginx.svc.cluster.local
pod "test-ns" deleted
----
...
.*3. Pod lookup*
----
$ kubectl run test-ns --image=busybox:1.28 --rm -it --restart=Never -- nslookup dns-demo-2.dns-demo.nginx.svc.cluster.local
Server:    10.12.0.10
Address 1: 10.12.0.10 kube-dns.kube-system.svc.cluster.local

Name:      dns-demo-2.dns-demo.nginx.svc.cluster.local
Address 1: 10.8.0.9 dns-demo-2.dns-demo.nginx.svc.cluster.local
pod "test-ns" deleted
----

[source, bash]
.*4. Pod lookup(short name)*
----
$ kubectl run test-ns --image=busybox:1.28 --rm -it --restart=Never -- nslookup dns-demo-2.dns-demo.nginx
Server:    10.12.0.10
Address 1: 10.12.0.10 kube-dns.kube-system.svc.cluster.local

Name:      dns-demo-2.dns-demo.nginx
Address 1: 10.8.0.9 dns-demo-2.dns-demo.nginx.svc.cluster.local
pod "test-ns" deleted
----
...
* link:services/svc.yaml[services/svc.yaml]
...
NAME          TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)   AGE
backend-svc   ClusterIP   10.12.12.170   <none>        80/TCP    61s
----

[source, bash]
.*3. Access Service*
----
$ kubectl run test --image=curlimages/curl --rm -it --restart=Never -- curl http://backend-svc.backend.svc.cluster.local/foo
<br>F5 Demo App
    ...
    Server IP: 10.8.1.17
    Server Port: 8080
    Server Hostname: backend-v1-677c668965-xj2b8

    Client IP: 10.8.1.19
    Client Port: 47564
    Client Hostname: 10.8.1.19

    Session: EC9F446A2D38D0DE06AB317200A28740

    X-Forwarded-For: null

    Cookies:

    Request Headers: host: [backend-svc.backend.svc.cluster.local] user-agent: [curl/8.0.1-DEV] accept: [*/*]
----
...
.*1. Create Service*
----
kubectl apply -f services/nodeport-svc.yaml
----

* link:services/nodeport-svc.yaml[services/nodeport-svc.yaml]
...
$ kubectl get svc -n backend
NAME          TYPE       CLUSTER-IP     EXTERNAL-IP   PORT(S)        AGE
backend-svc   NodePort   10.92.12.169   <none>        80:30100/TCP   2m33s
----

[source, bash]
.*3. Access Service*
----
$ NODEIP=$(kubectl get nodes -o wide --no-headers | head -n 1 | awk '{print $6}')
$ kubectl run test --image=curlimages/curl --rm -it --restart=Never -- curl http://$NODEIP:30100/foo
<br>F5 Demo App
    ...
    Server IP: 10.88.0.6
    Server Port: 8080
    Server Hostname: backend-v1-677c668965-wdb42

    Client IP: 10.88.0.1
    Client Port: 45992
    Client Hostname: 10.88.0.1

    Session: 9B1910B5E65E69B82EF9566552F86B75

    X-Forwarded-For: null

    Cookies:

    Request Headers: host: [10.128.0.3:30100] user-agent: [curl/8.0.1-DEV] accept: [*/*]
----

=== LoadBalancer Service

[source, bash]
.*1. Reserve a Regional IP address*
----
gcloud compute addresses create regional-lb --region=us-central1
----

[source, bash]
.*2. Create Service*
----
kubectl apply -f services/lb-svc.yaml
----

* link:services/lb-svc.yaml[services/lb-svc.yaml]

NOTE: The loadBalancerIP in above file should be replaced with the output of commands `gcloud compute addresses describe regional-lb --region us-central1 --format json | jq -r '.address'`.

[source, bash]
.*3. View Service*
----
$ kubectl get svc backend-lb-svc -n backend
NAME             TYPE           CLUSTER-IP    EXTERNAL-IP      PORT(S)        AGE
backend-lb-svc   LoadBalancer   10.92.7.134   34.134.182.180   80:30250/TCP   2m2s
----
...
<br>F5 Demo App
    ...
    Server IP: 10.88.0.8
    Server Port: 8080
    Server Hostname: backend-v2-689b558544-cph8w

    Client IP: 10.88.0.1
    Client Port: 32896
    Client Hostname: 10.88.0.1

    Session: A9B56E7FB49041864F4A0C69F2ACDB72

    X-Forwarded-For: null

    Cookies:  

    Request Headers: host: [34.134.182.180] user-agent: [curl/7.64.1] accept: [*/*] 
----
...
[
  {
    "creationTimestamp": "2023-04-24T17:14:11.195-07:00",
    "description": "{\"kubernetes.io/service-name\":\"backend/backend-lb-svc\"}",
    "healthChecks": [
      "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/global/httpHealthChecks/k8s-cd972525ecc36141-node"
    ],
    "id": "1301113646279730620",
    "instances": [
      "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/zones/us-central1-a/instances/gke-standard-cluster-default-pool-7dd8856b-9tct",
      "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/zones/us-central1-a/instances/gke-standard-cluster-default-pool-7dd8856b-htfr"
    ],
    "kind": "compute#targetPool",
    "name": "a24bdfb9321ed41f0a305bf15ac16a59",
    "region": "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/regions/us-central1",
    "selfLink": "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/regions/us-central1/targetPools/a24bdfb9321ed41f0a305bf15ac16a59",
    "sessionAffinity": "NONE"
  }
]
----
...
[
  {
    "IPAddress": "34.134.182.180",
    "IPProtocol": "TCP",
    "creationTimestamp": "2023-04-24T17:14:15.031-07:00",
    "description": "{\"kubernetes.io/service-name\":\"backend/backend-lb-svc\"}",
    "fingerprint": "LQTIFOKmEK8=",
    "id": "9134871846559851960",
    "kind": "compute#forwardingRule",
    "labelFingerprint": "42WmSpB8rSM=",
    "loadBalancingScheme": "EXTERNAL",
    "name": "a24bdfb9321ed41f0a305bf15ac16a59",
    ...
    "portRange": "80-80",
    "region": "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/regions/us-central1",
    "selfLink": "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/regions/us-central1/forwardingRules/a24bdfb9321ed41f0a305bf15ac16a59",
    "target": "https://www.googleapis.com/compute/v1/projects/playground-s-11-17534dca/regions/us-central1/targetPools/a24bdfb9321ed41f0a305bf15ac16a59"
  }
]
----
...
backend-ingress   <none>   *                 80      42s

$ kubectl get ingress backend-ingress -n backend -o yaml
...
status:
  loadBalancer:
    ingress:
...
.*4. Access Service*
----
$ curl http://35.227.194.95/foo
<br>F5 Demo App

    Request URI: /foo
    Protocol: HTTP/1.1

    Server IP: 10.88.1.10
    Server Port: 8080
    Server Hostname: backend-v1-677c668965-t55tk

    Client IP: 35.191.26.99
    Client Port: 62127
    Client Hostname: 35.191.26.99

    Session: 8F5DCDDB687B2D2DD6A6CF47C009FBF9

    X-Forwarded-For: 42.61.112.56, 35.227.194.95

    Cookies:  

    Request Headers: x-forwarded-proto: [http] x-cloud-trace-context: [91cc1fb52a9d70a9044e4bce7712f253/6774299831759440222] host: [35.227.194.95] connection: [Keep-Alive] x-forwarded-for: [42.61.112.56, 35.227.194.95] user-agent: [curl/7.64.1] accept: [*/*] via: [1.1 google] 

$ curl http://35.227.194.95/bar
<br>F5 Demo App

    Request URI: /bar
    Protocol: HTTP/1.1

    Server IP: 10.88.1.11
    Server Port: 8080
    Server Hostname: backend-v2-689b558544-6t672

    Client IP: 35.191.3.146
    Client Port: 55131
    Client Hostname: 35.191.3.146

    Session: 16EA6F1BD5946DCEE692F930EA57020A

    X-Forwarded-For: 42.61.112.56, 35.227.194.95

    Cookies:  

    Request Headers: x-forwarded-proto: [http] x-cloud-trace-context: [02424b817533161a929785ddd6e7fc34/3775972147683202747] host: [35.227.194.95] connection: [Keep-Alive] x-forwarded-for: [42.61.112.56, 35.227.194.95] user-agent: [curl/7.64.1] accept: [*/*] via: [1.1 google] 
----

*5. View the NEGs*

|===
|kind |name |network |subnetwork |networkEndpointType |size

|networkEndpointGroup
|k8s1-77c041c5-backend-bar-svc-80-0d837087
|default
|default
|GCE_VM_IP_PORT
|2
...
|networkEndpointGroup
|k8s1-77c041c5-backend-foo-svc-80-b44a2712
|default
|default
|GCE_VM_IP_PORT
|2
...
|networkEndpointGroup
|k8s1-77c041c5-kube-system-default-http-backend-80-76fd5895
|default
|default
|GCE_VM_IP_PORT
|1
...
* link:services/network-endpoint-groups.json[services/network-endpoint-groups.json]

*6. View NEG Endpoints and K8S Pod*
...
[
  {
    "networkEndpoint": {
      "instance": "gke-standard-cluster-default-pool-6dc54375-5p12",
      "ipAddress": "10.88.0.8",
      "port": 8080
    }
  },
  {
    "networkEndpoint": {
      "instance": "gke-standard-cluster-default-pool-6dc54375-9kjv",
      "ipAddress": "10.88.1.11",
      "port": 8080
    }
  }
]
...
[source, bash]
----
backend-v2-689b558544-4z2bw   1/1   Running   0     96m   10.88.0.8    gke-standard-cluster-default-pool-6dc54375-5p12   <none>   <none>
backend-v2-689b558544-6t672   1/1   Running   0     96m   10.88.1.11   gke-standard-cluster-default-pool-6dc54375-9kjv   <none>   <none>
----
...
[source, bash]
----
backend-v1-677c668965-22d9x   1/1   Running   0     96m   10.88.0.7    gke-standard-cluster-default-pool-6dc54375-5p12   <none>   <none>
backend-v1-677c668965-t55tk   1/1   Running   0     96m   10.88.1.10   gke-standard-cluster-default-pool-6dc54375-9kjv   <none>   <none>
----
...
[source, json]
----
l7-default-backend-6dc845c45d-thj9v   1/1   Running   0     100m   10.88.1.5   gke-standard-cluster-default-pool-6dc54375-9kjv   <none>   <none>
----

*7. View the backend service*

|===
|kind |name |loadBalancingScheme |backends

|backendService
|k8s1-77c041c5-backend-bar-svc-80-0d837087
|EXTERNAL
|networkEndpointGroups/k8s1-77c041c5-backend-bar-svc-80-0d837087

|backendService
|k8s1-77c041c5-backend-foo-svc-80-b44a2712
|EXTERNAL
|networkEndpointGroups/k8s1-77c041c5-backend-foo-svc-80-b44a2712

|backendService
|k8s1-77c041c5-kube-system-default-http-backend-80-76fd5895
|EXTERNAL
|networkEndpointGroups/k8s1-77c041c5-kube-system-default-http-backend-80-76fd5895

|===

* link:services/backend-services.json[services/backend-services.json]
...
[
  {
    "creationTimestamp": "2023-04-25T00:17:13.964-07:00",
    "defaultService": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/backendServices/k8s1-77c041c5-kube-system-default-http-backend-80-76fd5895",
    "fingerprint": "fX873t5lCjU=",
    "hostRules": [
      {
        "hosts": [
          "*"
    ...
    "id": "2722661810240667798",
    "kind": "compute#urlMap",
    "name": "k8s2-um-czetktm6-backend-backend-ingress-2hdi1pys",
    "pathMatchers": [
      {
        "defaultService": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/backendServices/k8s1-77c041c5-kube-system-default-http-backend-80-76fd5895",
        ...
        "pathRules": [
          {
            ...
            "service": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/backendServices/k8s1-77c041c5-backend-foo-svc-80-b44a2712"
          },
          {
            "paths": [
              "/bar"
            ],
            "service": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/backendServices/k8s1-77c041c5-backend-bar-svc-80-0d837087"
          }
        ]
      }
    ],
    "selfLink": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/urlMaps/k8s2-um-czetktm6-backend-backend-ingress-2hdi1pys"
  }
]
----
...
[
  {
    "creationTimestamp": "2023-04-25T00:17:15.822-07:00",
    "description": "{\"kubernetes.io/ingress-name\": \"backend/backend-ingress\"}",
    "fingerprint": "_W8lraQNslI=",
    "id": "6306077953007206548",
    "kind": "compute#targetHttpProxy",
    "name": "k8s2-tp-czetktm6-backend-backend-ingress-2hdi1pys",
    "selfLink": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/targetHttpProxies/k8s2-tp-czetktm6-backend-backend-ingress-2hdi1pys",
    "urlMap": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/urlMaps/k8s2-um-czetktm6-backend-backend-ingress-2hdi1pys"
  }
]
----

*10. View Forward Rule*

[source, json]
----
[
  {
    "IPAddress": "35.227.194.95",
    "IPProtocol": "TCP",
    "creationTimestamp": "2023-04-25T00:17:17.819-07:00",
    "description": "{\"kubernetes.io/ingress-name\": \"backend/backend-ingress\"}",
    "fingerprint": "gY_fiMcgKlc=",
    "id": "2998012228011737234",
    "kind": "compute#forwardingRule",
    "labelFingerprint": "42WmSpB8rSM=",
    "loadBalancingScheme": "EXTERNAL",
    "name": "k8s2-fr-czetktm6-backend-backend-ingress-2hdi1pys",
    "networkTier": "PREMIUM",
    "portRange": "80-80",
    "selfLink": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/forwardingRules/k8s2-fr-czetktm6-backend-backend-ingress-2hdi1pys",
    "target": "https://www.googleapis.com/compute/v1/projects/playground-s-11-40b0f27a/global/targetHttpProxies/k8s2-tp-czetktm6-backend-backend-ingress-2hdi1pys"
  }
]
----
